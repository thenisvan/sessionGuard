...
import sessionGuard.entities.AnalyzerRequestResponse;
import sessionGuard.entities.OriginalRequestResponse;
import sessionGuard.entities.Session;
import sessionGuard.gui.dialog.DataExportDialog;
import sessionGuard.gui.util.BypassCellRenderer;
import sessionGuard.gui.util.CustomRowSorter;
import sessionGuard.gui.util.PlaceholderTextField;
import sessionGuard.gui.util.RequestTableModel;
import sessionGuard.gui.util.RequestTableModel.Column;
import sessionGuard.util.BypassConstants;
import sessionGuard.util.CurrentConfig;
import sessionGuard.util.Diff_match_patch;
import sessionGuard.util.GenericHelper;
import sessionGuard.util.Diff_match_patch.Diff;
import sessionGuard.util.Diff_match_patch.LinesToCharsResult;
...
public class CenterPanel extends JPanel {
	...
	public CenterPanel(MainPanel mainPanel) {
		...
		JButton filterButton = new JButton("Table Filters");
		filterButton.setToolTipText("Open table status and search filters");
		...
		JButton settingsButton = new JButton("Table Columns");
		settingsButton.setToolTipText("Show or hide columns");
		...
		JPanel quickFilters = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 5));
		JToggleButton sameToggle = new JToggleButton(BypassConstants.SAME.getName(), true);
		JToggleButton similarToggle = new JToggleButton(BypassConstants.SIMILAR.getName(), true);
		JToggleButton differentToggle = new JToggleButton(BypassConstants.DIFFERENT.getName(), true);
		JToggleButton naToggle = new JToggleButton(BypassConstants.NA.getName(), true);
		java.util.function.Consumer<Void> syncFromToggles = v -> {
			showBypassed.setSelected(sameToggle.isSelected());
			showPotentialBypassed.setSelected(similarToggle.isSelected());
			showNotBypassed.setSelected(differentToggle.isSelected());
			showNA.setSelected(naToggle.isSelected());
			tableModel.fireTableDataChanged();
		};
		sameToggle.addActionListener(e -> syncFromToggles.accept(null));
		similarToggle.addActionListener(e -> syncFromToggles.accept(null));
		differentToggle.addActionListener(e -> syncFromToggles.accept(null));
		naToggle.addActionListener(e -> syncFromToggles.accept(null));
		quickFilters.add(new JLabel("Status Filters:"));
		quickFilters.add(sameToggle);
		quickFilters.add(similarToggle);
		quickFilters.add(differentToggle);
		quickFilters.add(naToggle);

		JPanel northPanel = new JPanel(new BorderLayout());
		northPanel.add(tableControlPanel, BorderLayout.NORTH);
		northPanel.add(quickFilters, BorderLayout.SOUTH);
		...
		clearTableButton = new JButton("Clear Results");
		clearTableButton.addActionListener(e -> clearTablePressed());
		tableConfigPanel.add(clearTableButton);
		JButton exportDataButton = new JButton("Export Results");
		...
		// Hidden: changeMessageViewButton, syncTabCheckBox, showDiffCheckBox, expandDiffButton
		changeMessageViewButton.setVisible(false);
		syncTabCheckBox.setEnabled(false);
		syncTabCheckBox.setVisible(false);
		showDiffCheckBox.setEnabled(false);
		showDiffCheckBox.setVisible(false);
		expandDiffButton.setVisible(false);
