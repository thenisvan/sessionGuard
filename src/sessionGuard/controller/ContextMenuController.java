package sessionGuard.controller;
...
import sessionGuard.gui.dialog.RepeatRequestFilterDialog;
import sessionGuard.gui.entity.SessionPanel;
import sessionGuard.gui.entity.TokenPanel;
import sessionGuard.gui.main.ConfigurationPanel;
...
public class ContextMenuController implements IContextMenuFactory {
	...
	public List<JMenuItem> createMenuItems(IContextMenuInvocation invocation) {
		...
		if(invocation.getSelectedMessages() != null && invocation.getSelectedMessages().length > 0) {
			...
			addSessionMonitoringMenu(authAnalyzerMenu, invocation);
		}
	...
	private void addSessionMonitoringMenu(JMenu authAnalyzerMenu, IContextMenuInvocation invocation) {
		IHttpRequestResponse[] messages = invocation.getSelectedMessages();
		if(messages == null || messages.length == 0) {
			return;
		}
		JMenu monitoringMenu = new JMenu("Session Monitoring");

		if(messages.length == 1) {
			JMenu applyValidationMenu = new JMenu("Apply Validation Template");
			for(String sessionName : configurationPanel.getSessionNames()) {
				JMenuItem item = new JMenuItem("Session: " + sessionName);
				IHttpRequestResponse message = messages[0];
				item.addActionListener(e -> applyValidationTemplate(sessionName, message));
				applyValidationMenu.add(item);
			}
			if(applyValidationMenu.getItemCount() > 0) {
				monitoringMenu.add(applyValidationMenu);
			}
		}

		if(monitoringMenu.getItemCount() > 0) {
			authAnalyzerMenu.addSeparator();
			authAnalyzerMenu.add(monitoringMenu);
		}
	}

	private void applyValidationTemplate(String sessionName, IHttpRequestResponse message) {
		SessionPanel sessionPanel = configurationPanel.getSessionPanelByName(sessionName);
		if(sessionPanel == null || message == null || message.getRequest() == null) {
			return;
		}
		IRequestInfo requestInfo = BurpExtender.callbacks.getHelpers().analyzeRequest(message);
		String url = requestInfo.getUrl() != null ? requestInfo.getUrl().toString() : "";
		...
		if(requestBytes != null && requestBytes.length > requestInfo.getBodyOffset()) {
			body = new String(Arrays.copyOfRange(requestBytes, requestInfo.getBodyOffset(), requestBytes.length));
		}
		sessionPanel.applyValidationTemplate(url, method, headerBuilder.toString(), body);
		configurationPanel.setSelectedSession(sessionName);
		GenericHelper.animateBurpExtensionTab();
	}
